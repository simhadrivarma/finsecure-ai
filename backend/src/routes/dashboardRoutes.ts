...
const relatedCustomerFilter = (rows: any[]) => {
  ...
  rows.forEach((row) => {
    [row.customerId, row.id].forEach((value) => {
      if (value) ids.add(String(value));
    });

    [row.email, row.customerEmail, row.userEmail].forEach((value) => {
      if (value) emails.add(String(value).toLowerCase());
    });

    [row.accountNumber, row.accountNo, row.fromAccount].forEach((value) => {
      if (value) accounts.add(String(value).toUpperCase());
    });

    [row.customer, row.customerName, row.name].forEach((value) => {
    ...
    try {
      ...
      const riskDistribution = {
        normal: transactions.filter(
          (item: any) =>
            String(item.risk || "").toLowerCase() === "normal"
        ).length,
      ...
      const recentTransactions = transactions.slice(0, 5).map((item: any) => ({
        ...
        status: item.status,
        risk: item.risk,
        riskScore: Number(item.riskScore || 0) || 0,
        date: item.date,
        time: item.time,
      }));
